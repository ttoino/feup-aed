...
TEST(test_1, b_loseGame) {
    ...
    game1.addKid(c5);
    game1.addKid(c6);
    string s = "Pim Pam Pum cada bola mata um pra galinha e pro peru quem se "
               "livra es mesmo tu";
...
TEST(test_1, c_rearrange) {
    ...
    if (res.size() == 9) {
        vector<string> resok = {"Ana",  "Rui",  "Joao", "Maria", "Vasco",
                                "Luis", "Rita", "Hugo", "Pedro"};
...
TEST(test_1, d_removeOlder) {
    ...
    list<Kid>::iterator it =
        find(kidsOlder.begin(), kidsOlder.end(), Kid("Rui", 6, 'm'));
...
TEST(test_2, FindMin) {
    ...
    int x_el[] = {5000000,  6000000,  7000000,  8000000, 9000000,
    ...
    // NOTE: parameter 10 means that values in the range [-10, 10] are
    // considered constant
