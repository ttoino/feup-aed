...
bool isConstant(const unsigned int y_el[], const int size, const int var) {
    ...
    int primDif = 0; // para ser constante o valor de referencia da diferen�a
                     // entre elementos dever� ser zero
    // cout << "primDif = " << primDif << endl;
    int dif = 0;
    for (int i = 1; i < size; i++) {
        dif = (y_el[i] - y_el[i - 1]);
        // cout << "dif = " << dif << endl;
        // cout << "primdif-var = " << primDif-var << "primdif+var = " <<
        // primDif+var << endl;
        if (dif < (primDif - var) || dif > primDif + var)
            return false;
    }
    return true;
    ...
    /*
        ...
        int primDif = 0; // para ser constante o valor de referencia da
       diferenca entre elementos devera ser zero cout << "primDif = " << primDif
       << endl; int soma = 0; int dif = 0; for (int i = 1; i < size; i++) { dif
       = (y_el[i] - y_el[i-1]); soma = soma + dif; cout << "dif = " << dif <<
       endl; cout << "soma = " << soma << endl;
        }
        int media = soma/(size-1);
        cout << "media = " << soma/(size-1) << " primdif-var = " << primDif-var
       << " primdif+var = " << primDif+var << endl; return (media >= primDif-var
       && media <= primDif+var);
    */
}
...
bool isLinear(const unsigned int y_el[], const int size, const int var) {
    ...
    // entre elementos consecutivos, somar essas diferencas e dividir pelo
    // numero de elementos -1. O resultado tera de estar dentro do intervalo
    // pretendido

    // Primeiro, verificar se ee ou nao constante
    if (isConstant(y_el, size, var))
        return false;
...
bool isQuadratic(const unsigned int y_el[], const int size, const int var) {
    ...
    // Para saber se ee quadratica, primeiro garantir que nao ee constante nem
    // linear. Depois, Calcular as segundas diferencas entre elementos
    // consecutivos, somar essas diferencas e dividir pelo numero de elementos
    // -2. O resultado tera de ser igual a diferenca da primeira
    // segunda-diferenca
    //
    //
    // Primeiro, verificar se ee ou nao constante (de notar que nao ee
    // suficiente comparar com Linear, pois nao ser linear pode significar que
    // ee constante)
    if (isConstant(y_el, size, var))
        return false;
    // Segundo, verificar se ee ou nao linear
    if (isLinear(y_el, size, var))
        return false;
...
bool isCubic(const unsigned int y_el[], const int size, const int var) {
    ...
    // Para saber se ee cubica, primeiro garantir que nao ee constante nem
    // linear nem quadratica. Depois, Calcular as terceiras diferencas entre
    // elementos consecutivos, somar essas diferencas e dividir pelo numero de
    // elementos -3. O resultado tera de ser igual a diferenca da primeira
    // segunda-diferenca
    //
    //
    // Primeiro, verificar se e ou nao constante (de notar que nao ee suficiente
    // comparar com Linear, pois nao ser linear pode significar que ee
    // constante)
    if (isConstant(y_el, size, var))
        return false;
    // Segundo, verificar se ee ou nao linear
...
bool isExponential(const unsigned int y_el[], const int size, const int var) {
    ...
    // entre elementos consecutivos, somar essas diferencas e dividir pelo
    // numero de elementos -1. O resultado tera de ser igual ao valor da
    // primeira divisao
