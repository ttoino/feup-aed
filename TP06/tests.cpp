...
TEST(test_1, b_printDictionary) {
    ...
    cout << "This test never fails. Check the information on the monitor"
         << endl;
    Dictionary d1;
    d1.readFile(f);
    f.close();
...
TEST(test_1, d_updateDictionary) {
    ...
    EXPECT_EQ(false,
              d1.update("esquilo", "pequeno mamifero roedor de cauda longa"));
    BST<WordMean> dicT = d1.getWords();
    bool isEmp = dicT.isEmpty();
    EXPECT_EQ(false, isEmp);
    if (isEmp == false) {
        ...
        EXPECT_EQ("pequeno mamifero roedor de cauda longa",
                  d1.consult("esquilo", prev, next));
    }
}
