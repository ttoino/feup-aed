...
template <class T> void DisjointSets<T>::makeSet(const T &x) {
    a[x].parent = x;
    a[x].myrank = 0;
}

// Find the representative of the set of element x
template <class T> T DisjointSets<T>::find(const T &x) {
    if (a[x].parent != x)
...
template <class T> void DisjointSets<T>::unite(const T &x, const T &y) {
    T xRoot = find(x);
    T yRoot = find(y);
