...
template <class T> T &BTNode<T>::getElement() { return element; }

template <class T> BTNode<T> *BTNode<T>::getLeft() const { return left; }

template <class T> BTNode<T> *BTNode<T>::getRight() const { return right; }
...
public:
    ...
    bool isEmpty() const;
    T &getRoot() const;
    BTNode<T> *getRootNode() const;
    void makeEmpty();
    void outputPreorder(ostream &out) const;
};

template <class T> BinaryTree<T>::BinaryTree(const T &el) {
...
template <class T>
BinaryTree<T>::BinaryTree(const T &el, const BinaryTree<T> &l,
...
template <class T> T &BinaryTree<T>::getRoot() const {
    if (root)
        return root->element;
    throw Underflow();
}

template <class T> BTNode<T> *BinaryTree<T>::getRootNode() const {
    return root;
}

template <class T> void BinaryTree<T>::makeEmpty() { makeEmpty(root); }

template <class T> void BinaryTree<T>::makeEmpty(BTNode<T> *n) {
    if (n) {
        makeEmpty(n->left);
        makeEmpty(n->right);
        delete n;
...
template <class T> void BinaryTree<T>::outputPreorder(ostream &out) const {
...
template <class T> ostream &operator<<(ostream &out, const BinaryTree<T> &t) {
...
template <class T> BTItrPost<T>::BTItrPost(const BinaryTree<T> &t) {
    if (!t.isEmpty())
        slideDown(t.root);
}

template <class T> void BTItrPost<T>::advance() {
    itrStack.pop();
    visitStack.pop();
...
template <class T> void BTItrPost<T>::slideDown(BTNode<T> *n) {
    while (n) {
        itrStack.push(n);
        if (n->left) {
...
template <class T> BTItrPre<T>::BTItrPre(const BinaryTree<T> &t) {
    if (!t.isEmpty())
...
template <class T> BTItrIn<T>::BTItrIn(const BinaryTree<T> &t) {
    if (!t.isEmpty())
        slideLeft(t.root);
}
...
template <class T> BTItrLevel<T>::BTItrLevel(const BinaryTree<T> &t) {
    if (!t.isEmpty())
        itrQueue.push(t.root);
}

template <class T> void BTItrLevel<T>::advance() {
