...
TEST(test_1, insert) {
    ...
    EXPECT_FALSE(
        h.insert(42)); // Já inserido, não deve conseguir voltar a inserir
    EXPECT_TRUE(h.insert(22));
    EXPECT_TRUE(h.insert(50));
    EXPECT_TRUE(h.insert(32));
    EXPECT_FALSE(
        h.insert(62)); // Tabela já com 4 elementos: mais um iria encher
    ...
    for (string s : data)
        EXPECT_FALSE(h2.contains(s));
    for (string s : data)
        EXPECT_TRUE(h2.insert(s));
    for (string s : data)
        EXPECT_FALSE(h2.insert(s)); // Já inseridos
    for (string s : data)
        EXPECT_TRUE(h2.contains(s));
...
TEST(test_1, remove) {
    ...
    EXPECT_FALSE(h.insert(15)); // Ao inserir ficava sem posicoes empty (1
                                // deleted + 3 Active ja na tabela)
...
TEST(test_2, dnaMOtifs) {
    ...
    EXPECT_EQ(2, FunWithHashTables::dnaMotifs("GTATATAGCGACTGATCGATGCTA", 3,
                                              motifs4));
...
TEST(test_3, findLanguage) {
    ...
    FunWithHashTables::findLanguage(
        "Algoritmos e Estruturas de Dados: uma UC de LEIC", dict1, answer1);
    ...
    answer2.clear();
    FunWithHashTables::findLanguage("quarenta e dois: o sentido da vida", dict2,
                                    answer2);
    correct2 = {{"pt", 4}, {"es", 1}, {"en", 0}, {"fr", 0}};
    EXPECT_EQ(answer2, correct2);

    answer2.clear();
    FunWithHashTables::findLanguage("--life--vida--vie--", dict2, answer2);
    correct2 = {{"pt", 1}, {"es", 1}, {"en", 1}, {"fr", 1}};
    EXPECT_EQ(answer2, correct2);

    answer2.clear();
    FunWithHashTables::findLanguage(
        "cuarenta y dos cuarenta y dos  -  dos   y   cuarenta", dict2, answer2);
    ...
    answer3.clear();
    FunWithHashTables::findLanguage("dUnE: FEAR is ThE MiNd killer!", dict3,
                                    answer3);
    correct3 = {{"pt", 1}, {"en", 4}, {"us", 4}};
    EXPECT_EQ(answer3, correct3);

    answer3.clear();
    FunWithHashTables::findLanguage("atreides harkonnen", dict3, answer3);
...
TEST(test_4, wordIndex) {
    ...
    answer.clear();
    FunWithHashTables::wordIndex({"a", "ab", "abc", "abcd", "abcde"}, answer);
    correct = {1, 27, 352, 2952, 17902};
    EXPECT_EQ(answer, correct);

    answer.clear();
    FunWithHashTables::wordIndex({"drtv", "ans", "yz", "e", "cimo", "demos"},
                                 answer);
    correct = {10511, 578, 351, 5, 8277, 51014};
    EXPECT_EQ(answer, correct);

    answer.clear();
    FunWithHashTables::wordIndex({"fhjk", "knqtw", "tuv", "z", "km"}, answer);
